...
// Gates access with a Gumroad license key, then calls Claude with a server-side
...
export default async function handler(req, res) {
  ...
  // 1. Verify the license key against Gumroad
  let verifyData;
  try {
  ...
  // Gumroad increments `uses` automatically on each verify call. At Haiku
  // pricing, even 200 quotes on one key costs well under $1, so this exists
  // purely to block a runaway script or bot, not to ration real buyers.
  const maxUses = 200;
  if (verifyData.uses && verifyData.uses > maxUses) {
